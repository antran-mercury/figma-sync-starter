...
| Scenario | When to use | Prompt file |
|----------|-------------|-------------|
| **Bootstrap mapping** (one-time, existing project) | You have a working codebase and want to establish a `figma-mapping.json` baseline for future incremental syncs. | [`.prompts/prompt-03-bootstrap-mapping.md`](.prompts/prompt-03-bootstrap-mapping.md) |
| **Init project** (first-time Figma → Code) | Starting from scratch — convert Figma frames into production-ready components and set up the sync workflow. | Generated by `yarn ai:init` → `figma/prompts/<date>-init-prompt.md` |
| **Sync updates** (incremental, after Figma change) | Figma was updated — apply only the changed nodes to your codebase. | Generated by `yarn ai:sync` → `figma/prompts/<date>-sync-prompt.md` |
